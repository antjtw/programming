...
const m1w1 = [
  {id:"lower",jp:"下半身",name:"Lower",focus:"Wake glutes, graded L-adductor, knee/ankle IR for stacking",blocks:[
    ...
    {t:"Main",items:[
      M("Squat","SSB or high-bar, 3-0-2","3×8","50% / RPE 5","100","Wide, toes-forward (your stacked stance). Knees track forward OVER the foot, not past it — stay tall. Above pain-free depth.","high bar squat tempo"),
...
const WU = {
  squat:[
    ...
    M("Copenhagen — adductor rehab","current stage","per protocol","≈3×/wk",null,"Tracked adductor rehab — see Movement Goals › L-Adductor. Carries into the squat bottom.","copenhagen plank progression","L-adductor"),
    M("Spanish squat","warm hold","1×30s","RPE 5",null,"Knee/tendon prime. Shallow if R-knee grumbles.","spanish squat isometric","R-knee"),
    M("Weighted knee-to-wall","dorsiflexion","2×10/side",null,null,"Knee forward over the toe, heel down. Buys the forward knee travel for a taller, wide squat.","weighted knee to wall ankle"),
  ...
  sumo:[
    ...
    M("Copenhagen — adductor rehab","current stage","per protocol","≈3×/wk",null,"Tracked adductor rehab — see Movement Goals › L-Adductor. Direct sumo carryover.","copenhagen plank progression","L-adductor"),
...
const m2Sessions = [
  { id:"lower-a", jp:"下A", name:"Lower A — Squat focus", focus:"Paused comp squat + spine-sparing leg volume", wu:"squat",
    main:[
      _mm("Competition squat","paused","paused squat","Wide, toes-forward. 2s pause — own the bottom. Knees forward OVER the foot, stay tall (not shoved past it). Deliberately re-earns the deep loaded position for the L-adductor (see L-Adductor protocol).","L-adductor",
...
const m3Sessions = [
  { id:"sq", jp:"スク", name:"Squat Day", focus:"Top double @7-8 + fatigue back-offs", wu:"squat",
    main:[
      ...
      _mm("Paused squat","secondary","paused squat","Eccentric ownership through the bottom — re-earning the deep loaded position for the L-adductor (see protocol).","L-adductor",
...
WU.fb = [
  ...
  M("Copenhagen — adductor rehab","current stage","per protocol","≈3×/wk",null,"TRACKED target now, not just prehab — work your current stage from Movement Goals › L-Adductor. Advance on criteria; finish it this time.","copenhagen plank progression","L-adductor"),
  ...
  M("Weighted knee-to-wall","dorsiflexion","2×10/side",null,null,"PRIORITY now. Drive the knee forward over the toe, heel down — this is what unlocks the upright wide squat.","weighted knee to wall ankle"),
...
const m15 = {
  a:{ id:"fb-a", jp:"全A", name:"Full Body A — Squat", focus:"SSB wide/tempo squat (upright rebuild) + light sumo groove + upper", blocks:[
    { t:WARM, items:WU.fb },
    { t:"Main", items:[
      M("SSB squat","wide · heel-elevated · 3-1-1 tempo","3×6","RPE 6",null,"Rebuild the upright, stacked wide squat. Heel-elevated to let the knees travel FORWARD over the foot; SSB keeps you tall. Knees over the foot, not past it — stay upright. Heel lift is a training-wheel while dorsiflexion catches up.","safety bar squat heel elevated"),
...
const PROG = {
  ...
  mesos:[
    ...
    },
    {
      ...
      targets:"End-of-meso ceilings (not goals): SSB/high-bar squat ~100×8 @6 · bench ~70×8 @6 · trap-bar (blocks) ~140×6 @6.",
      weeks:(function(){
  ...
  goals:[
    {jp:"外反",area:"Knee Valgus Pattern",tag:"primary",problem:"Knees cave in under load; tibial internal rotation, medial arch (MLA) collapse, ankle pronation — the pattern in the standing photo.",why:"It reads as a foot problem but it's driven from the hip: weak posterior glute med and deep hip external rotators failing to control femoral internal rotation. The arch collapse is downstream.",fix:["Glute med + deep hip ER strength (clamshells → crab walks → side-lying abduction → SL-RDL)","Foot intrinsics: short-foot drill, single-leg balance","NOT orthotics — propping the arch passively just moves the problem upstream","Under the bar, stack the knee OVER the foot — not caved in, and (for you) not shoved past it either. See Squat Structure."],time:"6-10 weeks of consistent integration to see visible change."},
    {jp:"軌道",area:"Squat Structure — Stance & Knee Travel",tag:"primary",problem:"At the bottom of the squat the knees drift OUTWARD, past the foot — the opposite of the valgus cave — losing the stacked structure. It shows only at the deepest range and only since narrowing the stance; at a genuinely wide, toes-forward stance the knees stay stacked. Separately, the wide squat is very hip-dominant and tipped forward.",why:"A stance-vs-structure mismatch, not a mobility block — heel elevation didn't change the drift. The hips are built for a wide, sit-back squat (vertical shins, little knee travel, depth from the hips). A narrower stance runs out of hip-flexion room at the bottom, so the femurs abduct to buy depth and the knees spill outside the foot. The forward lean is the same coin: minimal knee travel keeps the hips back and the torso folded. Likely the same story as the L-adductor strain — a knee drifting out at depth loads the adductor, lengthened, exactly where it fails.",fix:["Wide, toes-forward is the primary / competition stance — where the knees stay stacked. Narrow drops to a light secondary at most.","Retire 'spread the floor, knees out' — it now feeds the drift. New cue: knees forward and over the foot, stay tall. Stack over the foot, not past it.","Prioritise ankle dorsiflexion (see Ankle) — the job now is to unlock forward knee travel so the torso sits more upright in the wide stance. Wide is more forgiving on the ankle than narrow.","Groove the pattern light: SSB / tempo / paused / heel-elevated wide squats through M1.5-M2. Heel elevation is a training-wheel for the upright pattern here, not a fix for the drift.","Accept a structural ceiling: chase more upright than now (clearly available), but if the torso won't rise further with good ankle range and a grooved pattern, that's anatomy — don't force it."],time:"Groove at low load through M1.5-M2; the wide, upright, stacked squat should be the default heavy squat by M3."},
    ...
    {jp:"内転",area:"L-Adductor — Recurrent Strain (2022)",tag:"managed",problem:"L adductor that strains in the deepest part of the squat. First popped on a heavy deep squat in the wide, toes-forward stance in ~2022. Since then it nags and tightens persistently, with a handful of minor re-strains — rarely a full pop, but never fully settled.",why:"A recurrent, incompletely-rehabbed strain, not an acute tweak. It failed at end range under load and the original rehab stopped early — leaving a strength deficit at length, exactly where it was injured. That drives the cycle: it tightens protectively, stretching soothes but doesn't fix the deficit, and end-range load re-provokes it. The stance is NOT the cause — it happened in the strong wide stance. (18 years compensating around the R-knee + restricted R-hip IR likely loaded the L side more, but the unfinished rehab is why it persists.)",fix:["Treat it as a rehab to COMPLETE, not prehab — progressive adductor strength through to the lengthened, deep position. Staged progression below.","Don't just stretch it — stretching eases the tightness but doesn't build the missing end-range strength.","Re-earn the deep loaded squat bottom deliberately (paused WIDE squats), don't tiptoe around it.","Stance stays wide, toes-forward — it isn't the problem, so don't narrow to avoid it."],protocol:{stages:[{n:"Stage 1 · Build tolerance (isometric)",items:["Copenhagen plank, SHORT lever (knee on bench) — 3× hold, build 15s → 30s per side","Isometric adduction squeeze (ball / cushion between knees) — 3×30s"],advance:"Short-lever 3×30s/side, pain-free, no next-day soreness."},{n:"Stage 2 · Lengthen + early eccentric",items:["Copenhagen plank, LONG lever (ankle on bench) — 3× hold, build 10s → 30s per side","Slider / Valslide adduction, eccentric-focus (slow out, assist back) — 3×6-8 per side"],advance:"Long-lever 3×20-30s controlled; sliders pain-free through range."},{n:"Stage 3 · Full raises + loaded adduction",items:["Copenhagen RAISE, full (lower under control, raise back) — 3×6-10 per side","Cable / band standing adduction through FULL range, incl. the lengthened end — 3×10-12 per side"],advance:"Full Copenhagen 3×8+/side; cable adduction strong and symmetrical L vs R."},{n:"Stage 4 · Load + re-earn the deep squat",items:["Copenhagen with top-leg raise, or added ankle load — 3×6-8 per side","Loaded adduction (heavier cable / machine) — 3×8 per side","Paused WIDE squat into the deep position, progressively loaded — the transfer to the demand that first broke it (built through the squat programming, RPE-capped)"],advance:"Runs into the strength blocks. The step most people skip — the deep loaded position must feel strong and confident, not avoided."}],completion:"Strong and symptom-free through full range under load — INCLUDING the deep loaded squat bottom — with the L side matching the R in Copenhagen capacity, and no nag/tighten cycle for several weeks. Only then drop to maintenance (Copenhagen ~2×/week). Do not abandon it the moment the nagging stops — that is exactly when it's been dropped before, and why it's still here.",cadence:"Run the current stage ~3×/week on its own progression, separate from the day's main lifting. Advance only when the criterion is met — on capability, not the calendar."},redflags:["Sharp, specific pain (distinct from the 'working' stretch/burn) — drop back a stage.","Pain lingering beyond 24-48h, or worsening session to session — deload and reassess.","A true acute 'pop' — sudden sharp pull with loss of strength — stop, treat it as an acute strain, and get it assessed.","Pain that's more central / pubic than in the muscle belly, worse with coughing or sneezing, or feels bony — this can be adductor-related groin or pubic-bone involvement (managed differently) — get it assessed.","No meaningful progress after 4-6 weeks of consistent work — get it assessed."],time:"No fixed timeline — advance on the criteria, not the calendar. Expect the stages to run across M1.5-M2 and into the strength blocks."},
    ...
    {jp:"足首",area:"Ankle Mobility",tag:"managed",problem:"Limited dorsiflexion. Now a priority — it's the rate-limiter on the upright-wide squat rebuild.",why:"Forward knee travel is exactly what dorsiflexion allows. To sit more upright in the wide stance, the knees need to travel forward over the foot — and that's blocked by the stiff ankle. (It doesn't cause the outward drift, though: heel elevation didn't change that, so the drift is a stance issue, not an ankle one.)",fix:["Daily DF work, prioritised: banded distraction, weighted knee-to-wall (knee forward over the toe, heel down)","Calf stretch with rear foot turned slightly IN (addresses ER compensation)","Heel-elevated wide squats as a training-wheel while the ankle opens — groove the upright, knee-forward pattern now, wean the elevation as range improves","Wide stance buys more knee travel per degree of ankle range than narrow — another reason to stay wide"],time:"Small daily dose, disproportionate payoff. Front-loaded in M1.5-M2 to enable the squat rebuild."},
